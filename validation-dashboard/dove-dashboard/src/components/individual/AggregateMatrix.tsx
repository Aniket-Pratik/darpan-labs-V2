...
export function AggregateMatrix({ data }: Props) {
  const {
  ...
  return (
    <div className="bg-darpan-surface border border-darpan-border rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-white">Participant × Concept Matrix</h3>
                  ...
                  {CONCEPT_NAMES.map((_, cIdx) => {
                    ...
                    const val = concept
                      ? getMetricValue(
                          {
                            mae: concept.mae,
                            accuracy: concept.plus_minus_1_accuracy,
                            exact: concept.exact_match_rate,
                          },
                          matrixMetric
                        )
                      : null;
                ...
                <span
                  ...
                  style={{
                    color: qualityColor(
                      getMetricValue(
                        {
                          mae: data.aggregate.overall_mae,
                          accuracy: data.aggregate.overall_accuracy,
                          exact: data.aggregate.overall_exact,
                        },
                        matrixMetric
                      ),
                      matrixMetric
