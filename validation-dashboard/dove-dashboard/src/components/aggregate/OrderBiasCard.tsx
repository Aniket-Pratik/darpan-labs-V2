...
export function OrderBiasCard({ data }: Props) {
  ...
  const verdictColor = mm.verdict.includes('Minimal')
    ? '#00FF88'
    : mm.verdict.includes('Moderate')
      ? '#FFB800'
      : '#FF4444';
  ...
  return (
    <div className="bg-darpan-surface border border-darpan-border rounded-xl p-5">
      ...
      <p className="text-[10px] text-white/40 mt-3 leading-relaxed">
        Variance decomposition shows how much of the score variation is due to
        the concept itself vs. presentation order vs. individual respondent
        differences.
      </p>
    </div>
  );
